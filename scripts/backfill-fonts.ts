/**
 * Re-runs `extractFontFamilies` over captured_email rows and writes the
 * resulting list into `metadata.font_families`.
 *
 * Use this once after deploying the font extractor so historical rows
...
import { extractFontFamilies, type FontFamily } from "../lib/extract-metadata";
...
function existingFontsLength(metadata: Json | null): number {
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) {
    return 0;
  }
  const candidate = (metadata as Record<string, unknown>).font_families;
...
async function processRow(
  ...
  try {
    ...
    const previousLength = existingFontsLength(row.metadata);

    if (fonts.length === 0 && previousLength === 0) {
      return { kind: "unchanged", id: row.id, length: 0 };
    }

    if (opts.dryRun) {
      return { kind: "updated", id: row.id, fonts, previousLength };
    }
...
async function main(): Promise<void> {
  ...
  if (!opts.allRows) {
    query = query.is("metadata->font_families", null);
  }
  ...
  console.log(
    `Font backfill plan: ${rows.length} row(s) | dry-run=${opts.dryRun} | scope=${opts.allRows ? "all" : "metadata.font_families IS NULL"} | concurrency=${opts.concurrency}`
  );
  ...
  await runWithConcurrency(rows, opts.concurrency, async (row, index) => {
    ...
    switch (outcome.kind) {
      case "updated": {
        updated += 1;
        totalFonts += outcome.fonts.length;
        const previewFamilies = outcome.fonts
          .slice(0, 5)
          .map((f) => f.family)
          .join(", ");
        console.log(
          `${position} ${outcome.id} ${outcome.previousLength} -> ${outcome.fonts.length} fonts  ${previewFamilies}`
        );
        break;
      }
      case "unchanged":
        unchanged += 1;
        console.log(
          `${position} ${outcome.id} ${outcome.length} fonts — unchanged`
        );
        break;
      case "skipped":
        skipped += 1;
  ...
  if (updated > 0) {
    const avg = (totalFonts / updated).toFixed(1);
    console.log(`  avg fonts:      ${avg} per updated row`);
  }
  if (opts.dryRun) {
...
main().catch((error) => {
  console.error("Font backfill crashed:", error);
  process.exit(1);
});
