...
--
-- Welcome emails are excluded from the calculation: they are signup-triggered
-- automation (often a multi-message flow fired within minutes of subscribing),
-- not the brand's ongoing broadcast cadence, so counting them collapses the
-- first-to-last span and grossly inflates the per-brand rate. The 5-email
-- threshold is therefore applied to non-welcome emails only.
...
as $$
  with brand_stats as (
    ...
    where c.deleted_at is null
      and e.category is distinct from 'welcome'
