-- User-metrics rollup for the admin "Users" tab.
--
-- Returns a single jsonb blob (mirrors pirol_admin_dashboard_stats) covering
-- growth by tier, churn / retention recency, product-market-fit proxies, and an
-- activation funnel. The heavy lifting stays in Postgres so the admin client
-- gets everything in one round trip.
--
-- SECURITY DEFINER because it aggregates across *all* users — per-user RLS on
-- user_profiles / saved_emails / collections would otherwise hide every row
-- from the calling admin. We gate access inside the function with an
-- admin_users check on auth.uid(), so a non-admin role can't read it directly.
--
-- "Team" accounts (rows in admin_users) are excluded from churn / PMF / funnel
-- denominators so internal usage doesn't flatter the numbers; the tier counts
-- still surface them separately. Activity is measured off user_profiles
-- .last_active_at (the last app load), which is the only per-user activity
-- signal we persist — buckets are therefore "time since last seen".
...
begin
  ...
  -- One pass over the real (non-team) users for every filtered count.
  rs as (
    select
      count(*) as total,
  ...
  -- Cumulative signups (all users) and cumulative paid conversions over time.
  bounds as (
